...
st.set_page_config(
    page_title="TPL N-1 Study Sandbox",
    layout="wide",
)
...
def sync_base_case_generator_columns(base_cases_df: pd.DataFrame, gens_df: pd.DataFrame) -> pd.DataFrame:
    """Adds missing generator columns to base cases based on generator names."""
...
st.caption("Editable network model, add/remove base cases, automatic topology mapping, contingency analysis, and upgrade prioritization.")
...
with st.sidebar:
    ...
    st.divider()
    if st.button("Reset to Default 7-Bus Network"):
...
with tabs[0]:
    ...
    st.session_state.buses_df = st.data_editor(
...
with tabs[1]:
    ...
    st.session_state.lines_df = st.data_editor(
...
with tabs[2]:
    ...
    st.session_state.trafos_df = st.data_editor(
...
with tabs[3]:
    ...
    st.session_state.gens_df = st.data_editor(
...
with tabs[5]:
    ...
    st.session_state.ext_grids_df = st.data_editor(
...
with tabs[6]:
    ...
    st.info("Add/remove rows to add/remove base cases. Generator dispatch columns are synced from the generator table by generator name.")
    st.session_state.base_cases_df = sync_base_case_generator_columns(
        st.session_state.base_cases_df,
        st.session_state.gens_df,
    )
    st.session_state.base_cases_df = st.data_editor(
        st.session_state.base_cases_df,
        num_rows="dynamic",
        use_container_width=True,
        key="base_cases_editor",
    )
...
with tabs[7]:
    ...
    if st.button("Build Network and Run N-1 Study", type="primary"):
        try:
            ...
            summary_df, detail_df = run_multicase_n1_study(
                net,
                st.session_state.base_cases_df,
            ...
            st.session_state["original_loads"] = original_loads
    ...
    if "summary_df" in st.session_state:
        ...
        if upgrade_df.empty:
            ...
            base_case_row = st.session_state.base_cases_df[
                st.session_state.base_cases_df["case_name"] == representative_case_name
            ]
            ...
            if base_case_row.empty:
                ...
                try:
                    # Rebuild a clean network for the plot.
                    ...
                    st.download_button(
                        "Download Recommended Upgrade Plot",
                        data=buffer.getvalue(),
                        file_name="recommended_upgrade_plot.png",
                        mime="image/png",
                    )
                except Exception as e:
